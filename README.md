...
- 🧠 **실시간 정리** — 녹음 중에도 개요·타임라인·액션 아이템을 바로바로 미리 보여줌
- 🔇 **무음 모드** — 설정에서 실시간 자막을 끄면 음성 인식 알림음 없이 오디오만 녹음 (일부 안드로이드 폰의 시작/종료 비프음 회피)
- 🔒 **화면 꺼짐 방지** — 녹음 중 Screen Wake Lock으로 화면 자동 잠금을 막아 녹음이 계속되도록 유지 (백그라운드 복귀 시 자동 재획득). 단, 전원 버튼으로 화면을 강제로 끄거나 iOS에서 앱을 전환하면 OS가 캡처를 멈출 수 있음
